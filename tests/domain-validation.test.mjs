...
test('measures text limits in Unicode code points and accepts exact boundaries', () => {
  const normalized = normalizeCohortSubmission(validInput({
    ...
    topic: 't'.repeat(100),
    targetAudience: 'a'.repeat(4_000),
    additionalDetails: 'x'.repeat(4_000),
    minQuorum: 1,
...
test('rejects under/over limits, malformed email, bad enums, and unknown fields', () => {
  const invalidCases = [
    ['title', { title: 'ab' }],
    ['description', { description: 'x'.repeat(4_001) }],
    ['targetAudience', { targetAudience: 'x'.repeat(4_001) }],
    ['additionalDetails', { additionalDetails: 'x'.repeat(4_001) }],
