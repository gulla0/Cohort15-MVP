...
Provider-dependent production tasks must identify human setup requirements before relying on external setup. Every human-action checklist or runbook must live in `docs/human-tasks/`, include a `## Human Setup Checklist` section, and be registered in `docs/human-tasks/README.md`.
