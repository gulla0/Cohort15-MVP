...
| T003 | Add persistence schema and credit ledger primitives | done | yes | `npm run check` passed with persistence and credit ledger tests. | Added dependency-free in-memory repositories, schema metadata, demo grants, and auditable ledger helpers. |
| T004 | Build create cohort flow | done | yes | `npm run check` passed with create-flow service and route tests. | Added demo-backed create form, 2-credit creator hold, default expiry, validation surfacing, and hidden-link success rendering. |
...
| T006 | Build show-interest and quorum unlock flow | done | yes | `npm run check` passed; browser smoke tested create, stake interest, quorum activation, and unlocked private link on `http://localhost:3001`. | Added show-interest service, POST route, detail UI, quorum credit consumption, and consumed-participant link authorization. |
...
| T009 | Build creator and participant dashboards | done | yes | `npm run check` passed; browser smoke checked `/dashboard/creator` and `/dashboard/participant`. | Added dashboard data loader, routes, UI, credit summaries, and authorized unlocked-link rendering. |
...
| T012 | Replace demo user selection with regular auth boundary | done | yes | `npm run check` passed with 59 tests. | Added local session-cookie sign-in/sign-out, guarded create/interest/dashboard routes, removed protected-flow query/default demo identity paths, and scoped private-link viewing to the signed-in user. |
| T013 | Add credit purchase package flow | not_started | yes | Pending. | Depends on T011 and T012; start from `$6`/6-credit and `$12`/14-credit packages with local/mock payment unless a provider is selected. |
| T014 | Publish social outbox to configured external channels | not_started | yes | Pending. | Depends on T011 and T012; implement adapter-backed publishing with dry-run/mock mode and no private-link leakage. |
| T015 | Add cohort completion and cancellation lifecycle handling | not_started | yes | Pending. | Depends on T012; implement cancellation refunds and active cohort completion for already-modeled statuses. |
...
| T017 | Choose and document deployment target | not_started | yes | Pending. | Depends on T012 and T016; choose first host and document runtime, env, persistence, and deploy/runbook assumptions. |
...
| T022 | Complete launch privacy and security review | not_started | no | Pending. | Depends on T012, T018, T019, T020, and T021; verify auth, private-link handling, logs, admin operations, and credit integrity. |
