...
| Production deployment/config | T013-T014 in `tasks.json`, `plan.md`, `README.md`, `docs/deployment-render.md`, `docs/production-config.md`, `render.yaml`, `src/config/runtime.mjs`, `src/server/app.mjs` | Render Web Service `cohort15-mvp` is the first production deployment target. Use `https://cohort15-mvp.onrender.com` unless Render assigns a different URL. The root `cohort15.com` domain currently hosts the pre-release landing page on Netlify, but production launch can keep the Render URL, move `cohort15.com` to Render, or use a subdomain such as `https://app.cohort15.com`. Production mode is triggered by `COHORT15_APP_ENV=production` or `NODE_ENV=production`; startup validation rejects missing required provider/admin/session values, local JSON persistence, and `COHORT15_UPLOAD_MODE=local`. | provider credentials committed to repo |
| Supabase Auth | T015 and T018 in `tasks.json`, `src/auth/session.mjs`, `src/auth/supabase.mjs`, `src/server/app.mjs`, `src/ui/auth.mjs` | T015 is implemented with dependency-free Supabase OAuth/PKCE start and callback exchange. Production `/auth/sign-in` renders Google/GitHub provider links, `/auth/supabase/:provider` starts OAuth, `/auth/callback` converts the Supabase user to an app session, and seeded local sign-in is disabled in production. Optional email magic links are gated by `SUPABASE_ENABLE_MAGIC_LINK`; T018 still needs hardened session/CSRF behavior. | OAuth provider alternatives unless user changes decision |
| Supabase Postgres persistence | T017 in `tasks.json`, persistence modules, credit ledger modules | Production persistence uses Supabase Postgres and must preserve transaction-derived credit accounting. | local JSON as production store |
| Stripe credit purchases | T020-T021 in `tasks.json`, credit package assumptions in `plan.md`, credit ledger modules | Stripe is selected. `$6`/6-credit and `$12`/14-credit packages must be real payment-backed, webhook-verified, idempotent, and auditable. | mock-only payment for production |
...
| Buy Credits placeholder feedback | ISSUE-012 in `agent/feedback/issue-index.md`, `src/ui/home.mjs`, `src/server/app.mjs` | Resolved: primary app navigation uses shared `renderTopbar` with `Buy Credits`, and `/credits/buy` renders a clear coming-soon placeholder that collects no payment and grants no credits. Real Stripe payment gates and credit purchase transactions are now T020-T021. | changing placeholder behavior outside Stripe purchase tasks |
...
- T012 added dependency-free local session auth in `src/auth/session.mjs` with explicit seeded-user sign-in/out. T013 selected Render for deployment. T014 added production runtime configuration in `src/config/runtime.mjs`; T015 added Supabase Auth with Google/GitHub provider routing and callback session creation. The next production-MVP graph step is T017 Supabase Postgres before session hardening, Stripe, social, and admin launch tasks.
...
- Supabase Auth is the selected and implemented production auth provider for Google and GitHub login, but session cookie hardening and CSRF protection remain T018.
...
- Secret management and required production env vars are formalized in `docs/production-config.md`, `.env.example`, `render.yaml`, and `src/config/runtime.mjs`. Later provider-backed tasks should extend this boundary instead of reading `process.env` ad hoc.
...
- 2026-06-17 EDT: T013 chose Render Web Service `cohort15-mvp` as the first production deployment target, added `render.yaml` and `docs/deployment-render.md`, documented Node 24 runtime/start/health/rollback/app URL assumptions, and adjusted `src/server/app.mjs` to bind to `HOST` or `0.0.0.0`.
- 2026-06-17 EDT: T014 added production runtime configuration validation, `.env.example`, `docs/production-config.md`, Render secret placeholders, and tests for required production env values and rejected local fallbacks.
