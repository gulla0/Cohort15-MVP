...
| Production deployment/config | T013-T014 in `tasks.json`, `plan.md`, `README.md`, `docs/deployment-render.md`, `render.yaml`, `src/server/app.mjs` | Render Web Service `cohort15-mvp` is the first production deployment target. Use `https://cohort15-mvp.onrender.com` unless Render assigns a different URL. The root `cohort15.com` domain currently hosts the pre-release landing page on Netlify, but production launch can keep the Render URL, move `cohort15.com` to Render, or use a subdomain such as `https://app.cohort15.com`. Config/secrets boundary should fail clearly in production. | provider credentials committed to repo |
...
- README documents local demo users, seed credit grants, MVP flow, manual expiry trigger, local social outbox behavior, credit package assumptions, known MVP assumptions, and the T013 Render deployment target; production docs still need T014-T030 updates.
...
- T012 added dependency-free local session auth in `src/auth/session.mjs` with explicit seeded-user sign-in/out. T013 selected Render for deployment. The next production-MVP graph step is T014 config/secrets, followed by T015 Supabase Auth and T017 Supabase Postgres before Stripe/social/admin launch tasks.
...
- `cohort15.com` currently hosts the pre-release landing page on Netlify. Do not assume the final app URL until T014 or provider setup confirms it; valid options include the Render service URL, moving the root domain to Render, or using an app subdomain.
