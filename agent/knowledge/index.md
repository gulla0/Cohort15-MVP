...
| Production deployment/config | T013-T014 in `tasks.json`, `plan.md`, `README.md`, `src/server/app.mjs` | Choose the production target before finalizing Supabase callback URLs, Stripe webhooks, and social credentials; config/secrets boundary should fail clearly in production. | provider credentials committed to repo |
...
- README documents local demo users, seed credit grants, MVP flow, manual expiry trigger, local social outbox behavior, credit package assumptions, and known MVP assumptions; production docs still need T013-T030 updates.
...
- T012 added dependency-free local session auth in `src/auth/session.mjs` with explicit seeded-user sign-in/out. The production-MVP graph now starts at T013 deployment, T014 config/secrets, T015 Supabase Auth, and T017 Supabase Postgres before Stripe/social/admin launch tasks.
