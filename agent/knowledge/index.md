...
| Production deployment/config | T013-T014 in `tasks.json`, `plan.md`, `README.md`, `docs/deployment-render.md`, `render.yaml`, `src/server/app.mjs` | Render Web Service `cohort15-mvp` is the first production deployment target. Use `https://cohort15-mvp.onrender.com` unless Render assigns a different URL; optional custom domain assumption is `https://app.cohort15.com` after DNS verification. Config/secrets boundary should fail clearly in production. | provider credentials committed to repo |
