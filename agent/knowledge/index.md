...
| Buy Credits placeholder feedback | ISSUE-012 in `agent/feedback/issue-index.md`, `src/ui/home.mjs`, `src/server/app.mjs` | Resolved: primary app navigation uses shared `renderTopbar` with `Buy Credits`, and `/credits/buy` renders a clear coming-soon placeholder that collects no payment and grants no credits. Real payment gates and credit purchase transactions remain T013. | implementing payment provider or granting credits |
| Create-form placeholder contrast feedback | ISSUE-013 in `agent/feedback/issue-index.md`, `src/ui/create-cohort.mjs`, `src/ui/styles.css` | Not started: follow-up to ISSUE-010 for very light placeholder text that disappears when users focus/click into create-form fields. Preserve meaningful placeholder examples for unfocused empty fields. | changing validation, image upload, or broader create-form flow |
| Buy Credits nav alignment feedback | ISSUE-014 in `agent/feedback/issue-index.md`, `src/ui/home.mjs`, `src/ui/styles.css` | Not started: follow-up to ISSUE-012 for aligning the `Buy Credits` nav item with the other navbar options while keeping `/credits/buy` a non-payment placeholder. | payment implementation, credit grants, full nav redesign |
