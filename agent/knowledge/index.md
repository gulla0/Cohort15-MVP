...
| Supabase Auth | T015 and T018 in `tasks.json`, `src/auth/session.mjs`, `src/server/app.mjs`, `src/ui/auth.mjs` | Production auth uses Supabase Auth with Google and GitHub required, optional magic-link/email; local seeded auth should be development-only. | OAuth provider alternatives unless user changes decision |
...
- T012 added dependency-free local session auth in `src/auth/session.mjs` with explicit seeded-user sign-in/out. T013 selected Render for deployment. T014 added production runtime configuration in `src/config/runtime.mjs`; the next production-MVP graph step is T015 Supabase Auth, followed by T017 Supabase Postgres before Stripe/social/admin launch tasks.
...
- Supabase Auth is the selected production auth provider, but the current implemented boundary is still local session auth over seeded users until T015.
