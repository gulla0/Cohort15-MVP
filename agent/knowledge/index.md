...
| Create-form guidance and image upload feedback | ISSUE-010 in `agent/feedback/issue-index.md`, `src/ui/create-cohort.mjs`, `src/server/app.mjs` | Add meaningful placeholders first, then handle familiar local file-picker image selection with validation/default fallback. | full media library or cloud uploads |
| Fuzzy cohort search feedback | ISSUE-011 in `agent/feedback/issue-index.md`, `src/services/event-browsing.mjs`, `tests/event-browsing.test.mjs` | Build on ISSUE-008: exact matches first, typo-tolerant matches below, public-safe fields only. | hosted search service, private links |
