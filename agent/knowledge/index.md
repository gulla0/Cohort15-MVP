...
| Feedback tracking | `agent/feedback/*`, `templates/feedback-issue/*` | Issue-local feedback workflow exists. ISSUE-001, ISSUE-002, and ISSUE-003 were resolved on 2026-05-30. ISSUE-004 is open for dashboard information architecture, token summary copy, content-based dashboard labels, active schedule hierarchy, and dashboard flow review. | high |
...
- Open ISSUE-004 requests dashboard presentation cleanup: remove repeated row-level token information, show only Available/In use/Used in dashboard token summaries, replace role-heavy dashboard labels with content-based sections like Active Cohorts & Schedule, Created Cohorts, and Interested Cohorts, and review dashboard page hierarchy.
