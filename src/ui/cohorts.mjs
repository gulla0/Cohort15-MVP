...
function pageStart(title, googleAnalyticsId, socialMeta = '', auth = null, currentPath = '') {
  return `<!doctype html><html lang="en"><head>${analyticsMarkup(googleAnalyticsId)}<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">${socialMeta}<title>${escapeHtml(title)} | Cohort15</title><link rel="stylesheet" href="/assets/styles.css"></head><body>${renderSiteHeader({ auth, currentPath })}`;
}
...
export function renderCohortDetailPage(cohort, options = {}) {
  ...
  return `${pageStart(cohort.title, options.googleAnalyticsId ?? 'G-LF22TLDSBV', cohortSocialMeta(cohort, { appUrl: options.appUrl }), options.auth, `/cohorts/${cohort.id}`)}<main class="shell detail-shell">
