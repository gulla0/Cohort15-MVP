...
function interestForm(cohort, { error = false } = {}) {
  ...
  return `<section class="interest-panel" aria-labelledby="interest-heading">
    ...
    ${error ? '<p class="form-error" role="alert">Check your email and try again.</p>' : ''}
    <form class="interest-form" method="post" action="/cohorts/${encodeURIComponent(cohort.id)}/interests">
      ...
      <label for="interest-email">Email</label><input id="interest-email" name="email" type="email" autocomplete="email" required maxlength="254">
