...
function shell(title, auth, content, currentPath) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${title} | Cohort15</title><link rel="stylesheet" href="/assets/styles.css"></head><body>${renderSiteHeader({ auth, currentPath })}<main class="shell auth-shell">${content}</main></body></html>`;
}
...
export function renderCheckoutCompletePage({ auth, state, returnTo = '/' } = {}) {
  ...
  return shell('Checkout result', auth, `<section class="auth-card">${content}<div class="button-row"><a class="button-link" href="${escapeHtml(returnTo)}">Continue</a><a class="button-link secondary" href="/credits/buy">Buy credits</a></div></section>`, '/credits/checkout/complete');
}
