...
- [x] 1. Open the [Stripe Dashboard](https://dashboard.stripe.com/) and switch on **Test mode**.
- [x] 2. Go to **More** -> **Product catalog** -> **Products**, or open [Product catalog](https://dashboard.stripe.com/test/products).
- [x] 3. Create a product named `Cohort15 Credits` with two one-time USD prices:
   - `$6.00` for the 6-credit package.
   - `$12.00` for the 14-credit package.
- [x] 4. Confirm both prices have `price_...` identifiers available to copy later. Price IDs are configuration values, not secrets.
